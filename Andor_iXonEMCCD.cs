...
namespace QCam
{
	unsafe class Andor_iXonEMCCD : ICamera
	{
        #region DLL import
		...
		[DllImport("atmcd32d.dll", EntryPoint = "SetOutputAmplifier",
		   ExactSpelling = false, CallingConvention = CallingConvention.StdCall)]
		private static extern uint _SetOutputAmplifier(int typ);

		[DllImport("atmcd32d.dll", EntryPoint = "SetEMGainMode",
		   ExactSpelling = false, CallingConvention = CallingConvention.StdCall)]
		private static extern uint _SetEMGainMode(int mode);

		[DllImport("atmcd32d.dll", EntryPoint = "SetEMCCDGain",
		   ExactSpelling = false, CallingConvention = CallingConvention.StdCall)]
		private static extern uint _SetEMCCDGain(int gain);

		[DllImport("atmcd32d.dll", EntryPoint = "GetEMCCDGain",
		   ExactSpelling = false, CallingConvention = CallingConvention.StdCall)]
		private static extern uint _GetEMCCDGain(ref int gain);
        ...
        private const int READMODE = 4;	//0: Full Vertical Binning, 1: MultiTrack, 2: Random Track, 3: Single Track, 4: Image Mode
        ...
        public unsafe uint Open()
        {
			lock (codeLock)
			{
				...
				err = _SetEMGainMode(0);	// gain is between 0-255 (default)
				ReportError(err, "--> Set EM Gain Mode");
        ...
        public unsafe uint Set(string name, int value)
        {
			lock (codeLock)
			{
				...
				{
					err = _SetEMCCDGain(value);
					ReportError(err, "--> SetEMGain");
				}
				else if (name == "Amp")
				{
					err = _SetOutputAmplifier(value);
					if (err == SUCCESS)
					{
						amp = value;
						byte[] desc = new byte [21];
						_GetAmpDesc(amp, desc, 21);
						Console.WriteLine("--> Amp: " + Encoding.ASCII.GetString(desc));
					}
					ReportError(err, "--> SetAmp");
				}
        ...
        public unsafe short Get(string name)
        {
			lock (codeLock)
			{
				...
				{
					err = _GetEMCCDGain(ref wValue);
					ReportError(err, "--> GetEMGain");
				}
				else if (name == "Amp")
				{
					wValue = amp;
				...
				{
					_GetNumberHSSpeeds(0, amp, ref wValue);
				}
				else if (name == "FKVSS")
				{
					wValue = FKVSS;
		...
		public ushort[] ArrGet(string name)
		{
			lock (codeLock)
			{
				...
				{
					err = _GetNumberHSSpeeds(ADChannel, amp, ref number);
					...
					else
					{
						...
						for (int i = 0; i < number; i++)
						{
							_GetHSSpeed(ADChannel, amp, i, ref value);
